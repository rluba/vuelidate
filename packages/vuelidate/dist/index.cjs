...
function createValidationResults(rules, model, key, resultsCache, path, config, instance, externalResults, siblingState) {
  ...
  const $changed = vueDemi.ref(false);
...
function createMetaFields(results, nestedResults, childResults) {
  ...
  const $changed = vueDemi.computed({
    get() {
      return results.$changed.value || (allResults.value.length ? allResults.value.every(r => r.$changed) : false);
    },
    set(v) {
  ...
  const $anyChanged = vueDemi.computed(() => allResults.value.some(r => r.$changed) || allResults.value.some(r => r.$anyChanged) || $changed.value);
...
function setValidations({
  ...
  const cachedExternalResults = {
    ...(vueDemi.unref(externalResults) || {})
  };
  ...
  if (key && mergedConfig.$autoDirty) {
    vueDemi.watch(nestedState, () => {
      ...
      flush: 'sync',
      deep: mergedConfig.$autoDirtyDepth ?? false
    });
  }
...
function useVuelidate(validations, state, globalConfig = {}) {
  ...
  const instance = currentVueInstance || vueDemi.getCurrentInstance()?.proxy;
  ...
  return vueDemi.computed(() => {
    return {
      ...vueDemi.unref(validationResults.value),
      ...childResults.value
    };
  });
}
